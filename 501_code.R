...
# Subset data
data1 <- data[1:5, c(2, 3)] # stimulus strength vs. EMG amp.
data2 <- data[1:5, c(2, 4)] # stimulus strength vs. angle
data3 <- data[1:5, c(2, 5)] # stimulus strength vs. latency
...
plot_linear_regression <- function(data, x_col, y_col, title, point_color, line_color, xlab, ylab) {
  ...
  ggplot(data, aes(x = .data[[x_col]], y = .data[[y_col]])) +
    ...
    annotate("text", x = min(data[[x_col]]), y = max(data[[y_col]]), 
             ...
             size = 2.5, col = "black", family = "Helvetica") + # Annotation in Helvetica size 7
